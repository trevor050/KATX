...
const guideData = {
    ...
    function displayItems(items) {
        if(!itemGrid) return;
        itemGrid.innerHTML = ''; // Clear previous items and sentinel
        ...
        const isMobile = window.innerWidth <= 768;
        let currentItems = items; // Use a local copy for the closure
        let renderedCount = 0;
        const BATCH_SIZE = 15; // Number of items to load per batch on mobile
        let observer;
        let sentinel;

        function renderBatch(start, end) {
            const fragment = document.createDocumentFragment();
            const itemsToRender = currentItems.slice(start, end);
            itemsToRender.forEach(item => {
                const card = createItemCard(item);
                fragment.appendChild(card);
            });
            // Remove old sentinel before adding new content
            if (sentinel && sentinel.parentNode === itemGrid) {
                itemGrid.removeChild(sentinel);
            }
            itemGrid.appendChild(fragment);
            renderedCount = end; // Update the count of rendered items

            // Re-add sentinel if more items exist
            if (renderedCount < currentItems.length) {
                sentinel = document.createElement('div');
                sentinel.id = 'scroll-sentinel';
                sentinel.style.height = '100px'; // Give it some space to trigger intersection
                sentinel.style.gridColumn = '1 / -1'; // Span across grid columns
                itemGrid.appendChild(sentinel);
                observer.observe(sentinel); // Observe the new sentinel
            } else {
                // All items loaded, disconnect observer
                if (observer) {
                    observer.disconnect();
                    observer = null;
                }
            ...
            anime({
                targets: '.item-card:not(.animated)', // Target only new cards
                opacity: [0, 1],
                translateY: [15, 0],
                scale: [0.98, 1],
                ...
                easing: 'easeOutQuad',
                complete: (anim) => {
                    // Mark cards as animated to avoid re-animating
                    anim.animatables.forEach(a => a.target.classList.add('animated'));
                }
            });
        }
        ...
        if (isMobile) {
            ...
            const initialLoad = Math.min(currentItems.length, BATCH_SIZE);
            
            // Set up Intersection Observer
            observer = new IntersectionObserver((entries) => {
                entries.forEach(entry => {
                    if (entry.isIntersecting && renderedCount < currentItems.length) {
                        observer.unobserve(entry.target); // Stop observing the current sentinel
                        const nextBatchStart = renderedCount;
                        const nextBatchEnd = Math.min(renderedCount + BATCH_SIZE, currentItems.length);
                        renderBatch(nextBatchStart, nextBatchEnd);
                    }
                });
            }, { rootMargin: '200px 0px' }); // Load next batch when sentinel is 200px from viewport edge
