...
const guideData = {
    ...
    fusion: [
        ...
        function displayItems(items) {
            ...
            function handleIntersection(entries, observer) {
                entries.forEach(entry => {
                    ...
                    if (entry.isIntersecting) {
                        ...
                        if (targetElement.classList.contains('item-placeholder')) {
                            // console.log(`Placeholder ${index} intersecting, rendering card.`);
                            const newItemCard = createItemCard(currentItems[index]);
                        ...
                        if (targetElement.classList.contains('item-card')) {
                            // console.log(`Card ${index} leaving, rendering placeholder.`);
                            const newPlaceholder = createPlaceholderCard(index);
                            newPlaceholder.dataset.index = index; // Already set in create func, but good practice
            ...
            if (currentItems.length > 0 && !cardHeightMeasured) {
                try {
                    ...
                    if (rect.height > 50) { // Basic sanity check
                        ...
                        console.log(`Measured itemCardHeight: ${itemCardHeight}`);
                    } else {
            ...
            // --- Main Logic --- 
            if (isMobile) {
                console.log('--- Running Mobile Logic (IntersectionObserver) --- '); 
                ...
                itemObserver = new IntersectionObserver(handleIntersection, {
                    ...
                    rootMargin: '300px 0px', // Load/unload when item is 300px from viewport edge
                ...
                requestAnimationFrame(() => {
                    console.log('Inside requestAnimationFrame callback.'); // LOG 6: Inside RAF
                    updateVisibleItems(); // Initial call

                    // REMOVE SCROLL LISTENER - use setInterval instead
                    // Clear any existing interval first
                    if (window.cardUpdateInterval) {
                        clearInterval(window.cardUpdateInterval);
                    }
                    
                    // Set up a global interval to check visibility every 500ms
                    console.log('Setting up interval for updateVisibleItems');
                    // REDUCED INTERVAL from 500ms to 150ms
                    window.cardUpdateInterval = setInterval(() => {
                        // console.log('INTERVAL CHECK - calling updateVisibleItems'); // Optional: Can be noisy
                        updateVisibleItems();
                ...
                // Ensure previous intervals are cleared if resizing
                if (window.cardUpdateInterval) {
                    clearInterval(window.cardUpdateInterval);
                    window.cardUpdateInterval = null;
                }
