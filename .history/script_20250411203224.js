            ...
            function handleIntersection(entries, observer) {
                entries.forEach(entry => {
                    ...
                    if (entry.isIntersecting) {
                        ...
                        if (isPlaceholder) {
                            // Schedule card rendering during idle time
                            requestIdleCallback(() => {
                                // Double-check if still placeholder and in data range
                                const currentElementAtIndex = itemGrid.querySelector(`[data-index="${index}"]`);
                                if (currentItems[index] && currentElementAtIndex && currentElementAtIndex.classList.contains('item-placeholder')) {
                                    const newItemCard = createItemCard(currentItems[index]);
                                    newItemCard.dataset.index = index;
                                    
                                    observer.unobserve(targetElement); 
                                    itemGrid.replaceChild(newItemCard, targetElement);
                                    observer.observe(newItemCard); 
                                    renderedIndices.add(index);
                                }
                            }, { timeout: 500 }); // Timeout ensures it runs eventually
                        }
                    } else { // Not intersecting
                        // Element is leaving viewport (or buffer)
                        if (!isPlaceholder) { // Check if it's actually a card
                            // Schedule placeholder rendering during idle time
                            requestIdleCallback(() => {
                               // Double-check if still a card and in data range
                               const currentElementAtIndex = itemGrid.querySelector(`[data-index="${index}"]`);
                               if (currentItems[index] && currentElementAtIndex && currentElementAtIndex.classList.contains('item-card')) {
                                    const newPlaceholder = createPlaceholderCard(index);
