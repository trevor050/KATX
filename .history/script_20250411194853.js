...
const guideData = {
    ...
    function displayItems(items) {
        ...
        function updateVisibleItems() {
            ...
            const gridTopOffset = itemGrid.offsetTop; // Position relative to document top

            // Calculate first and last potentially visible row indices
            const firstVisibleRow = Math.max(0, Math.floor((scrollTop - gridTopOffset) / itemCardHeight) - columnCount); // Buffer above
            const lastVisibleRow = Math.ceil((scrollTop + viewportHeight - gridTopOffset) / itemCardHeight) + columnCount; // Buffer below

            const startIndex = Math.max(0, firstVisibleRow * columnCount);
            const endIndex = Math.min(currentItems.length - 1, (lastVisibleRow * columnCount) -1);
